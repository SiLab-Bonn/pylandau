...
def numpy_number(draw, min_val, max_val):
    ...
    if 'f' in dtype.str:
        if np.array(min_val) < np.finfo(dtype).min:
            min_val = np.finfo(dtype).min
        if np.array(max_val) > np.finfo(dtype).max:
...
class TestDatatypes():
    ...
    def test_landau_pdf_inputs(self, x, mpv, eta):
        ...
        assert (isinstance(pylandau.get_landau_pdf(x, mu=mpv,
                                                           eta=eta),
                                   float))
    ...
    def test_langau_pdf_inputs(self, x, mpv, eta, sigma):
        ...
        assert (isinstance(pylandau.get_langau_pdf(x, mu=mpv,
                                                           eta=eta,
                                                           sigma=sigma),
                                   float))
